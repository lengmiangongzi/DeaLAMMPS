...
//						  << "  --- cell position: " << cpos[0] << " " << cpos[1] << " " << cpos[2] << " " << std::endl;
	...
	void FEProblem<dim>::set_boundary_values()
	{
		...
		double tvel_vsupport=500.0; // target velocity of the boundary m/s-1
				...
				for (unsigned int q=0; q<quadrature_formula.size(); ++q)
				{
					...
					for(unsigned int repl=1;repl<nrepl+1;repl++)
					{
						...
						sprintf(filename, "%s/last.%d.%s_%d.dump", nanostatelocout, local_quadrature_points_history[q].qpid,
								local_quadrature_points_history[q].mat.c_str(), repl);
		                                sprintf(filenameout, "%s/lcts.%d.%s_%d.dump", nanostatelocres, local_quadrature_points_history[q].qpid,
		                                                local_quadrature_points_history[q].mat.c_str(), repl);
					...
					for (unsigned int q=0; q<quadrature_formula.size(); ++q)
					{
						...
						for(unsigned int repl=1;repl<nrepl+1;repl++)
						{
							...
							sprintf(filename, "%s/restart/lcts.%d.%s_%d.dump", nanostatelocin, local_quadrature_points_history[q].qpid,
									local_quadrature_points_history[q].mat.c_str(), repl);
							sprintf(filenameout, "%s/last.%d.%s_%d.dump", nanostatelocout, local_quadrature_points_history[q].qpid,
									local_quadrature_points_history[q].mat.c_str(), repl);
			                                sprintf(command, "cp %s %s 2>/dev/null", filename, filenameout);
                                			int ret = system(command);
                                			/*if (ret!=0){
