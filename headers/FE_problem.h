...
//        problem_type = (ProblemType<dim> *) new Dogbone<dim>(input_config);
    ...
    if ( triangulation.n_active_cells() < n_FE_processes &&
        ...
        dcout << "Exception: Cells < ranks in FE communicator... " << triangulation.n_active_cells() << ", " << n_FE_processes << "," << triangulation.n_active_cells() << ", " <<  n_world_processes <<
              std::endl;
        exit(1);
    }
...
void FEProblem<dim>::assign_microstructure (typename DoFHandler<dim>::active_cell_iterator cell, CellData<dim> celldata,
    ...
    Tensor<2, dim> idmat;
    idmat = 0.0;
    for (unsigned int i = 0; i < dim; ++i) idmat[i][i] = 1.0;
    ...
    for (unsigned int imd = 0; imd < mdtype.size(); imd++) {
        ...
        if (this_FE_process == 0) {
            ...
            printf("           %+.4e %+.4e %+.4e %+.4e %+.4e %+.4e \n", stiffness_tensors[imd][0][0][0][0], stiffness_tensors[imd][0][0][1][1], stiffness_tensors[imd][0][0][2][2],
                   stiffness_tensors[imd][0][0][0][1], stiffness_tensors[imd][0][0][0][2], stiffness_tensors[imd][0][0][1][2]);
            printf("           %+.4e %+.4e %+.4e %+.4e %+.4e %+.4e \n", stiffness_tensors[imd][1][1][0][0], stiffness_tensors[imd][1][1][1][1], stiffness_tensors[imd][1][1][2][2],
                   stiffness_tensors[imd][1][1][0][1], stiffness_tensors[imd][1][1][0][2], stiffness_tensors[imd][1][1][1][2]);
            printf("           %+.4e %+.4e %+.4e %+.4e %+.4e %+.4e \n", stiffness_tensors[imd][2][2][0][0], stiffness_tensors[imd][2][2][1][1], stiffness_tensors[imd][2][2][2][2],
                   stiffness_tensors[imd][2][2][0][1], stiffness_tensors[imd][2][2][0][2], stiffness_tensors[imd][2][2][1][2]);
            printf("           %+.4e %+.4e %+.4e %+.4e %+.4e %+.4e \n", stiffness_tensors[imd][0][1][0][0], stiffness_tensors[imd][0][1][1][1], stiffness_tensors[imd][0][1][2][2],
                   stiffness_tensors[imd][0][1][0][1], stiffness_tensors[imd][0][1][0][2], stiffness_tensors[imd][0][1][1][2]);
            printf("           %+.4e %+.4e %+.4e %+.4e %+.4e %+.4e \n", stiffness_tensors[imd][0][2][0][0], stiffness_tensors[imd][0][2][1][1], stiffness_tensors[imd][0][2][2][2],
                   stiffness_tensors[imd][0][2][0][1], stiffness_tensors[imd][0][2][0][2], stiffness_tensors[imd][0][2][1][2]);
            printf("           %+.4e %+.4e %+.4e %+.4e %+.4e %+.4e \n", stiffness_tensors[imd][1][2][0][0], stiffness_tensors[imd][1][2][1][1], stiffness_tensors[imd][1][2][2][2],
                   stiffness_tensors[imd][1][2][0][1], stiffness_tensors[imd][1][2][0][2], stiffness_tensors[imd][1][2][1][2]);
        }
...
void FEProblem<dim>::update_stress_quadrature_point_history(const Vector<double> &displacement_update,
    ...
    char time_id[1024];
    sprintf(time_id, "%d-%d", timestep, newtonstep);
    ...
    for (typename DoFHandler<dim>::active_cell_iterator
        ...
        if (cell->is_locally_owned()) {
            ...
            for (unsigned int q = 0; q < quadrature_formula.size(); ++q) {
                char cell_id[1024];
                sprintf(cell_id, "%d", local_quadrature_points_history[q].hist_strain.get_ID_to_update_from());
...
void FEProblem<dim>::clean_transfer() {
    ...
    for (typename DoFHandler<dim>::active_cell_iterator
        ...
        if (cell->is_locally_owned()) {
            ...
            for (unsigned int q = 0; q < quadrature_formula.size(); ++q) {
                char cell_id[1024];
                sprintf(cell_id, "%d", local_quadrature_points_history[q].qpid);
...
void FEProblem<dim>::output_lhistory () {
    ...
    for (typename DoFHandler<dim>::active_cell_iterator
        ...
        if (cell->is_locally_owned()) {
            char cell_id[1024];
            sprintf(cell_id, "%d", cell->active_cell_index());
