...
//    sprintf(mdstate, "g%d_%d", md_sim.material, md_sim.replica);
    ...
    sprintf(cline, "variable ll1 equal lx");
    lammps_command(lmp, cline);
    sprintf(cline, "variable ll2 equal ly");
    lammps_command(lmp, cline);
    sprintf(cline, "variable ll3 equal lz");
    lammps_command(lmp, cline);
    ...
    sprintf(cline, "variable dts equal %f", md_sim.timestep_length);
    lammps_command(lmp, cline);
    sprintf(cline, "variable nts equal %d", nts);
    lammps_command(lmp, cline);
    ...
    if (md_sim.checkpoint) {
        if (md_sim.force_field == "opls") {
            sprintf(cline, "write_restart %s", straindata_lcts);
            lammps_command(lmp, cline); /*opls*/
        } else if (md_sim.force_field == "reax") {
            sprintf(cline, "write_dump all custom %s id type xs ys zs vx vy vz ix iy iz", straindata_lcts);
            lammps_command(lmp, cline); /*reaxff*/
        }
    }
    // close down LAMMPS
    ...
    sprintf(cline, "reset_timestep 0");
    lammps_command(lmp, cline);
    ...
    if (md_sim.output_homog) {
        ...
        sprintf(cline, "dump atom_dump all atom %d %s", 1, homogdata_time);
        lammps_command(lmp, cline);
    }
    ...
    sprintf(cline, "variable nssample0 equal %d", md_sim.nsteps_sample);
    lammps_command(lmp, cline);
    sprintf(cline, "variable nssample  equal %d", md_sim.nsteps_sample);
    lammps_command(lmp, cline);
    ...
    if (md_sim.output_homog) {
        ...
        sprintf(cline, "undump atom_dump");
        lammps_command(lmp, cline);
    }
